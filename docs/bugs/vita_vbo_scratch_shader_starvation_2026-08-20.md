...
into. Vita now ends each frame with `vglSwapBuffers(GL_FALSE)` instead. The next
run did render correctly and reached a match without crashing, confirming that
the missing vitaGL swap was the black-screen cause. It also advances vitaGL's
frame state and rotates the circular allocation pool, which explains why the
earlier no-swap build eventually crashed in `_glDrawArrays_CustomShadersIMPL`.

The temporary nine-point `glReadPixels()` diagnostic and high-frequency frame
logs are removed from the normal path now that presentation is proven.

The Vita initializer was also still passing `SCE_GXM_MULTISAMPLE_4X` directly
to `vglInitWithCustomThreshold()`, despite the later port configuration and
diagnostic marker both selecting 1x. That GXM allocation cannot be downgraded
after initialization. It now uses `SCE_GXM_MULTISAMPLE_NONE`, removing the
unintended 4x render-target bandwidth and memory cost from every gameplay
frame.

## Startup visibility and persistent shader cache

The required 41-program prewarm can take two to three minutes on the first Vita
launch. It cannot simply be deferred: hardware logs prove that SceShaccCg fails
after the main archives consume the remaining compiler heap. The v4 build now
presents a shader-free clear/scissor progress bar before prewarm starts and
after every program, so the display becomes visibly active immediately.

vitaGL is rebuilt with `HAVE_SHADER_CACHE=1`. Successful source-hash GXP files
are stored under `ux0:data/shader_cache/SSB64VITA/v0/v` and `/f`; subsequent
launches retain the early prewarm ordering but load those binaries instead of
recompiling them. This is vitaGL's internal shader cache, not the rejected
application-level `glProgramBinary()` experiment. Its cache-write path also
checks for a successful compiler result and valid file handle before
serialization, preserving the graceful Shark failure handling.

The v4 hardware log must contain:

```text
SSB64: Vita present experiment v4 ... prewarm_progress=clearbar shader_cache=vitagl-gxp ...
SSB64: EARLY_SHADER_PREWARM complete success=41 failed=0 ...
```

Validate both a cold-cache and a warm-cache launch. The first must create GXP
files and visibly advance the bar; the second should reach the game materially
faster without shader-link failures.

## Release startup target

The release requirement is no more than 5--10 seconds of blank/progress display
before the game. A writable runtime cache only satisfies that target after the
first launch, so it is not sufficient for a clean install. The release VPK must
carry the GXP output collected from one complete real-hardware prewarm.

`Makefile.vita` packages `.gxp` files placed under
`port/vita_shader_cache/v0/{v,f}` as `app0:/shader_cache/v0/{v,f}`. Before
vitaGL initialization, the renderer checks for both packaged directories and
calls `vglSetShaderCachePath("app0:/shader_cache")`; otherwise it retains the
writable `ux0:` path used to generate the cache. This keeps development builds
self-seeding while making clean-install release builds avoid SceShaccCg.

Vita3K `shaderlog` files are not valid input for this bundle: they are emulator
pipeline dumps with unrelated names and serialization. The payload must come
from `ux0:data/shader_cache/SSB64VITA/v0/{v,f}` after the v5 Vita build reports
`EARLY_SHADER_PREWARM complete success=54 failed=0`.

A later four-minute v3 hardware log reached more gameplay states and exposed 13
additional programs after the original prewarm. All failed with Shark's fatal
internal error because newlib usage had already grown to roughly 110--115 MiB.
Those exact pairs are now included in the v5 early set, bringing the expected
marker to `count=54` and completion to `success=54 failed=0`. Only a cache
generated by this expanded run should be packaged for the startup target.
