...
static std::unordered_map<u32, unsigned long long> sPortRelocQuarantinedSources;
static std::unordered_set<u32> sPortRelocValidationRetries;
...
static void portRelocRetractPublication(u32 file_id, void *ram_dst, size_t size, int loc)
{
	...
	portPackedDisplayListCacheDeleteRange(ram_dst, size);
	portTextureCacheDeleteRange(ram_dst, size);
	portEvictStructFixupsInRange(ram_dst, size);
	portRelocInvalidateRange(ram_dst, size);
	portRelocEvictFileRangesInRange(ram_dst, size);
}
 ...
 * while it walks them, but historically a resource could still be published
 * with a token that no longer resolved into a registered live file range.
 * That turns a deterministic load problem into a later partial model / stale
 * pointer failure in Fast3D.  Validate every slot generated by this exact load
 * before RESOURCE_TRANSACTION_COMMIT makes the resource observable.
 */
static bool portRelocValidateTransactionSlots(void *ram_dst, size_t copy_size, u32 file_id,
                                              const std::vector<unsigned int> &slot_offsets,
                                              const char **out_reason, unsigned int *out_bad_offset,
                                              u32 *out_bad_token, void **out_bad_target)
{
	if (out_reason) *out_reason = nullptr;
	if (out_bad_offset) *out_bad_offset = 0;
	if (out_bad_token) *out_bad_token = 0;
	if (out_bad_target) *out_bad_target = nullptr;

	if (ram_dst == nullptr || copy_size == 0)
	{
		if (out_reason) *out_reason = "owner-empty";
		return false;
	}

	const PortRelocFileRange *owner = portRelocFindFileRangeAnyState(file_id, ram_dst);
	if (owner == nullptr || owner->size != copy_size)
	{
		if (out_reason) *out_reason = "owner-range-missing";
		return false;
	}

	const uintptr_t owner_base = reinterpret_cast<uintptr_t>(ram_dst);
	for (unsigned int off : slot_offsets)
	{
		if ((off & (sizeof(u32) - 1u)) != 0u ||
		    (size_t)off > copy_size || sizeof(u32) > copy_size - (size_t)off)
		{
			if (out_reason) *out_reason = "slot-oob";
			if (out_bad_offset) *out_bad_offset = off;
			return false;
		}

		const u32 token = *reinterpret_cast<const u32 *>(owner_base + off);
		void *target = token != 0 ? portRelocTryResolvePointer(token) : nullptr;
		if (token == 0 || target == nullptr)
		{
			if (out_reason) *out_reason = "token-unresolved";
			if (out_bad_offset) *out_bad_offset = off;
			if (out_bad_token) *out_bad_token = token;
			return false;
		}

		const uintptr_t target_addr = reinterpret_cast<uintptr_t>(target);
		const PortRelocFileRange *target_range = nullptr;
		for (auto it = sPortRelocFileRanges.rbegin(); it != sPortRelocFileRanges.rend(); ++it)
		{
			if (it->size != 0 && target_addr >= it->base &&
			    (target_addr - it->base) < it->size)
			{
				target_range = &*it;
				break;
			}
		...
		{
			if (out_reason) *out_reason = "target-unregistered";
			if (out_bad_offset) *out_bad_offset = off;
			if (out_bad_token) *out_bad_token = token;
			if (out_bad_target) *out_bad_target = target;
			return false;
		}

		/* External dependencies must already be fully committed.  The owner is
		 * the only range allowed to be not-ready while this transaction runs. */
		if (target_range->base != owner_base && !target_range->ready)
		{
			if (out_reason) *out_reason = "target-not-ready";
			if (out_bad_offset) *out_bad_offset = off;
			if (out_bad_token) *out_bad_token = token;
			if (out_bad_target) *out_bad_target = target;
			return false;
	...
	int             force_figatree_fixup)
{
	...
	{
		spdlog::error("lbReloc bridge: file_id {} data ({} bytes) exceeds buffer ({})",
		              file_id, copySize, bytes_num);
		port_log("SSB64: RESOURCE_PREFLIGHT_REJECT file_id=%u reason=allocation-too-small "
		         "input_size=%u allocation_size=%u action=quarantine\n",
	...
	{
		auto quarantined = sPortRelocQuarantinedSources.find(file_id);
		if (quarantined != sPortRelocQuarantinedSources.end())
		{
			if (quarantined->second == pristine_source_hash)
			{
				port_log("SSB64: RESOURCE_QUARANTINE_HIT file_id=%u src=%016llx path=%s action=reject-load\n",
				         file_id, pristine_source_hash, table_path ? table_path : "(mod)");
				return;
			}
			sPortRelocQuarantinedSources.erase(quarantined);
		}
	}
#endif
	...
	if (!load_ok)
	{
		portRelocRetractPublication(file_id, ram_dst, copySize, loc);
...
#ifdef __vita__
		if (sPortRelocValidationRetries.find(file_id) == sPortRelocValidationRetries.end())
		{
			port_log("SSB64: RESOURCE_VALIDATION_RETRY file_id=%u src=%016llx reason=%s "
			         "attempt=1 action=pristine-reload\n",
			         file_id, pristine_source_hash, failure_reason ? failure_reason : "unknown");
			sPortRelocValidationRetries.insert(file_id);
		...
		sPortRelocQuarantinedSources[file_id] = pristine_source_hash;
		port_log("SSB64: RESOURCE_QUARANTINE file_id=%u src=%016llx reason=%s "
		         "attempt=1 action=reject-publication\n",
		         file_id, pristine_source_hash, failure_reason ? failure_reason : "unknown");
#endif
		return;
	}
...
#ifdef __vita__
	// Build the read-only 3D provenance manifest after every relocation token is
	// live and before RESOURCE_READY. It validates owner-local G_DL edges for
	// segment-0E disambiguation. Exact Vtx ranges remain untouched here and are
	// committed only when a complete model consumer graph passes preflight.
...

	{
		...
		unsigned int bad_offset = 0;
		u32 bad_token = 0;
		void *bad_target = nullptr;
		if (!portRelocValidateTransactionSlots(ram_dst, copySize, file_id, reloc_slot_offsets,
		                                      &preflight_reason, &bad_offset, &bad_token, &bad_target))
		{
			port_log("SSB64: RESOURCE_PREFLIGHT_REJECT file_id=%u path=%s address=%p reason=%s "
			         "slot_off=0x%x token=0x%08x target=%p action=%s\n",
			         file_id, table_path ? table_path : "(mod)", ram_dst,
			         preflight_reason ? preflight_reason : "unknown",
			         bad_offset, (unsigned)bad_token, bad_target,
			         sPortRelocValidationRetries.find(file_id) == sPortRelocValidationRetries.end()
			             ? "pristine-reload" : "quarantine");

			portRelocRetractPublication(file_id, ram_dst, copySize, loc);
			port_log("SSB64: RESOURCE_LOAD name=%s size=%u address=%p reloc_count=%u status=FAIL "
			         "deps=%u reason=preflight\n",
			         table_path ? table_path : "(mod)", (unsigned)copySize, ram_dst,
			         (unsigned)(intern_steps + extern_steps), extern_count);
			if (sPortRelocValidationRetries.find(file_id) == sPortRelocValidationRetries.end())
			{
...
#ifdef __vita__
	{
		...
		else if (baseline_it->second.semantic_hash != semantic_hash)
		{
			port_log("SSB64: RESOURCE_INTEGRITY_MISMATCH file_id=%u expected=%016llx got=%016llx retry_depth=%d action=%s\n",
			         ...
			         sPortRelocValidationRetries.count(file_id) ? 1 : 0,
			         sPortRelocValidationRetries.count(file_id) == 0 ? "pristine-reload" : "quarantine");
			...
			sPortRelocQuarantinedSources[file_id] = pristine_source_hash;
			port_log("SSB64: RESOURCE_QUARANTINE file_id=%u src=%016llx reason=semantic-mismatch "
			         "expected=%016llx got=%016llx action=reject-publication\n",
			         file_id, pristine_source_hash, baseline_it->second.semantic_hash, semantic_hash);
			return;
		}
		else if (sPortRelocValidationRetries.find(file_id) != sPortRelocValidationRetries.end())
		{
			port_log("SSB64: RESOURCE_INTEGRITY_RECOVERED file_id=%u semantic=%016llx retry_depth=%d\n",
			         file_id, semantic_hash, 1);
		}
	}
#endif
	...
	unsigned short  reloc_intern_offset)
{
	...
	if (copySize > dst_size && dst_size > 0) {
		spdlog::error("portRelocLoadFileFromBytesPrivate: src ({} bytes) > dst ({})",
		              copySize, dst_size);
		port_log("SSB64: RESOURCE_PREFLIGHT_REJECT file_id=mod-private reason=allocation-too-small "
		         "input_size=%u allocation_size=%u action=reject-load\n", src_size, dst_size);
		return;
	}
...
{
	return portRelocFindContainingFile(ptr, out_base, out_size) ? 1 : 0;
}
